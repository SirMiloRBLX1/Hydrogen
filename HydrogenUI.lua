...
local KeySystem_Domain = "https://hydrogen-key-system.vercel.app"

local ClientID = tostring(game:GetService("Players").LocalPlayer.UserId)
local names = {"271635429", "1161221619" }

local ConfigURL = "https://raw.githubusercontent.com/SirMiloRBLX1/Hydrogen/refs/heads/main/Hydrogen.json"
local http_service = cloneref(game:GetService("HttpService"))
local HydrogenConfiguration = http_service:JSONDecode(game:HttpGet(ConfigURL))
...
if found then
    print("[ Hydrogen A+ ] Detecting Beta Users (Experimental Mode is ON)")
else
    print("Launching Old Hydrogen GUI")
end

if found then
	local new_ui = "https://raw.githubusercontent.com/SirMiloRBLX1/Hydrogen/refs/heads/main/HydrogenBeta.lua"
	local success, errorMessage = pcall(function()
		loadstring(game:HttpGet(new_ui))()
	end)
	if not success then
		warn("[Hydrogen Error]" .. errorMessage)
	end	
else
-- Launch Regular (Hydrogen)
--// Instances
local UserInputService = game:GetService("UserInputService")

local function HydrogenNotification(messages)
	local platform = UserInputService:GetPlatform()
	local platformName = "Unknown"

	if platform == Enum.Platform.Android then
		platformName = "Hydrogen Android"
	elseif platform == Enum.Platform.IOS then
		platformName = "Hydrogen iOS"
	else
		platformName = "Hydrogen"
	end

	game.StarterGui:SetCore("SendNotification", {
		Title = platformName;
		Text = messages;
		Icon = "rbxassetid://95158811637259";
		Duration = 5;
	})
end

local function HydrogenDebug(text)
...
-- 🔐 VALIDATOR FUNCTION
local function HydrogenCheckKey(key)
	local localPlayer = Players.LocalPlayer

	-- ✅ Developer bypass
	if localPlayer and localPlayer.UserId == DeveloperUserId then
		HydrogenNotification("✅ Developer bypass enabled")
		return true
	end

	-- ✅ Keyless toggle
	if IsKeyLess then
		HydrogenNotification("✅ Key system disabled")
		return true
	end

	-- ❌ Empty key
	if not key or key == "" then
		HydrogenNotification("❌ Please enter a key")
		return false
	end
	...
	if success then
		...
		if ok and data and data.data and data.data.expiresAt then
			HydrogenNotification("✅ Authorized | Key expires at:\n" .. data.data.expiresAt)
			return true
		end
	end

	HydrogenNotification("❌ Invalid or expired Hydrogen key")
	return false
end
...
if _isfolder("hydrogenui") == false then
	_makefolder("hydrogenui");
end
if _isfile("hydrogenui/settings.json") then
	isFirstTimeExecution = false;
	local succ, res = pcall(jsonDecode, httpService, _readfile("hydrogenui/settings.json"));
	if succ then
...
local function updateSettings(key, value)
	...
	pcall(_writefile, "hydrogenui/settings.json", jsonEncode(httpService, uiSettings));
end
...
local function loadKeyUI(callback)
	...
	-- local hydrogenAuth = loadstring(game:HttpGet("https://hydrogen-key-system.vercel.app"))()
	...
	local HydrogenAuth = loadstring(game:HttpGet('https://raw.githubusercontent.com/SirMiloRBLX1/Hydrogen/refs/heads/main/HydrogenLibBETA.lua'))()
	local url = HydrogenAuth:GetKey()
	_setclipboard(url)
	HydrogenNotification("Successfully Copied Key")
end)
 

		enterKey.MouseButton1Click:Connect(function()
local HydrogenAuth = loadstring(game:HttpGet("https://raw.githubusercontent.com/SirMiloRBLX1/Hydrogen/main/HydrogenLibBETA.lua"))()

local key = freeFrame.keyInput.Text
if HydrogenAuth.ValidateKey(key) then
	updateSettings("key", key)
	validLogin()
else
	HydrogenNotification("Invalid Key")
end
		end);
	...
	if HydrogenCheckKey(uiSettings.key) then
		validLogin();
	else
		HydrogenNotification("Saved Key is no longer valid")
	end
end
...
elseif platform == Enum.Platform.IOS then
	rconsoleprint("Hydrogen-iOS Successfully Loaded!\n")
else
	rconsoleprint("Hydrogen (Other Platform) Successfully Loaded!\n")
end
end;
